# -*- coding: utf-8 -*-
"""Implementation of the integrated gradients method.
...
def compute_integrated_gradients(mh, instances, target_classes,
    ...
    target_classes : int
        Target classes for which integrated gradients are computed.
    ...
    n_classes = mh.n_classes

    # get the representation features at the right layer
    if baseline is None:
        baseline = torch.zeros(instances.shape[1::])

    baseline = baseline.to(mh.device)
    target_classes = target_classes.to(mh.device)
    instances = instances.to(mh.device)
    ...
    out_grad[torch.arange(scaled_features.shape[0]), target_classes] = 1
    grads, (pred_classes, pred_probas) = mh.compute_gradients(
