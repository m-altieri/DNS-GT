...
def generate_oar_script(
    script_file_path,
    xp_var_name,
    task_ids=None,
    batch_size=1,
    oar_walltime='02:00:00',
    activate_env_command=None,
    use_gpu=False,
):
    ...
    script += (
        'python -c "import sys; sys.path.append(\'{0}\'); '
        'from {1} import {2}; '
...
def convert_results_into_dict(results, axes_labels, axes_values):
    """Convert the array of results into a dict of arrays.

    Keys of the dict are measure names, and values are the array of results for the
    specific measure.

    Parameters
    ----------
    results, axes_labels, axes_values : array-like
        Outputs of :meth:`load_results` of :mod:`yafe` experiment.
    ...
    # idm = axes_labels.index('measure')

    for idm, measure_name in enumerate(axes_values[-1]):
        dict_results[measure_name] = results[..., idm]

    return dict_results


def get_product_from_dict(param_dict):
    """Return a product of values from a dict.

    Parameters
    ----------
    param_dict : dict
        Dictionary keyed by parameter name and values by list of values.

    Returns
    -------
    list of dict
        Product of all combinations of values for each parameter.
    """
