...
class DELM(tf.keras.Model):
    ...
    def __init__(self, **conf):
        ...
        try:
            ...
            self._logger.warning(f'{Fore.RED}Could not open conf file {default_conf_file}:\n{e}' +
        ...
        self.domains_lookup = tf.keras.layers.StringLookup(vocabulary=self.conf['domains_vocab_path'],#domains_vocab,
                                                           num_oov_indices=0)
        self.hosts_lookup = tf.keras.layers.StringLookup(vocabulary=self.conf['hosts_vocab_path'],#hosts_vocab,
                                                         num_oov_indices=0)
        ...
        self.domain_embeddings = Embedding(input_dim=self.ndomains,
                                           output_dim=self.conf['domain_dim'],
                                           input_length=self.conf['sequence_length'])
                                           
        self.host_embeddings = Embedding(input_dim=self.nhosts,
                                         output_dim=self.conf['host_dim'],
                                         input_length=self.conf['sequence_length'])

        self.multihead_gat = MultiHeadGAT(n_heads=self.conf['n_heads'],
    ...
    def call(self, inputs, training=None):
        ...
        if training or self.conf['mask_test']:
            domains = self.mask(domains, p=tf.constant(self.conf['mask_p'], dtype=tf.float32))
    ...
    def slice_hosts(self, t):
        ...
        """
        return tf.slice(t, [0,0,0], tf.concat((tf.gather(tf.shape(t), [0,1]), [1]), axis=0))
    ...
    def slice_domains(self, t):
        """t must be a tf.Tensor of shape [Batch, Seqlen, 2]
        """
        return tf.slice(t, [0,0,1], tf.concat((tf.gather(tf.shape(t), [0,1]), [1]), axis=0))
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.compiled_loss(domain_indexes, pred, regularization_losses=self.losses)
...
class MultiHeadGAT(tf.keras.layers.Layer):
    ...
    def __init__(self, n_heads, emb_dim):
        ...
        self.Wq = [Dense(self.head_dim) for i in range(self.n_heads)]
        self.Wk = [Dense(self.head_dim) for i in range(self.n_heads)]
        self.Wv = [Dense(self.head_dim) for i in range(self.n_heads)]
        self.Wo = Dense(self.emb_dim)
        
        
    def call(self, inputs):
        
        # inputs (embeddings) [B, L, emb_dim]
        Q = tf.stack([Wqi(inputs) for Wqi in self.Wq], axis=1)  # [B, n_heads, L, head_dim]
        K = tf.stack([Wki(inputs) for Wki in self.Wk], axis=1)  # [B, n_heads, L, head_dim]
        V = tf.stack([Wvi(inputs) for Wvi in self.Wv], axis=1)  # [B, n_heads, L, head_dim]
        
        scores = tf.linalg.matmul(Q, tf.transpose(K, (0,1,3,2)))  # [B, n_heads, L, L]
        scores = tf.math.divide(scores, tf.math.sqrt(tf.cast(self.head_dim, tf.float32)))  # [B, n_heads, L, L] (normalization)
        scores = tf.nn.softmax(scores)  # [B, n_heads, L, L] attention weights
        result = tf.linalg.matmul(scores, V)  # [B, n_heads, L, head_dim]
        result = tf.concat(tf.unstack(result, axis=1), axis=-1)  # [B, L, n_heads*head_dim]
...
class Classifier(tf.keras.layers.Layer):
    ...
    def __init__(self, dense_dims, softmax_dim):
        ...
        self.dense_layers = [Dense(dim, activation='relu') for dim in dense_dims]
        
        self.out = Dense(softmax_dim, activation='linear')
