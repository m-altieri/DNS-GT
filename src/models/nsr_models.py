...
class NSRModel(tf.keras.Model):
    ...
    def __init__(self, H, D, **conf):
        ...
        self.nsr_hosts = tf.Variable(tf.random.uniform((H, self._host_dim)), 
                                     trainable=False,
                                     name='NSR Hosts',
                                     dtype=tf.dtypes.float32)
                                   
        self.nsr_domains = tf.Variable(tf.random.uniform((D, self._domain_dim)), 
                                       trainable=False,
                                       name='NSR Domains',
                                       dtype=tf.dtypes.float32)
        ...
        self.adj_hosts = tf.Variable(tf.ones((H, H)),
                                     trainable=False,
                                     name='Adj Hosts',
                                     dtype=tf.dtypes.float32)
                                   
        self.adj_domains = tf.Variable(tf.ones((D, D)),
                                       trainable=False,
                                       name='Adj Domains',
                                       dtype=tf.dtypes.float32)
