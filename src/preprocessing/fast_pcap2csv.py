...
def main(argv):
   ...
   argparser.add_argument('-o', '--output-folder', action='store', required=False, default='../../outputs/CSVs', dest='output_folder')
   ...
   for fname in pcap_files:
      ...
      for i, pkt in enumerate(pcap):
         if pkt.getlayer('DNS') and pkt.getlayer('DNS').opcode == 0:
            ...
            logger.error(f'{Fore.RED}{Style.BRIGHT}Packet {i} is not a DNS query.{Style.RESET_ALL}')
